...
double Mysecond(struct tm *ptr)
{
	return((Pi / 2) - (((double)ptr->tm_sec) / 60) * 2 * Pi);
}

double Mymin(struct tm *ptr)
{
	return((Pi / 2) - ((ptr->tm_min + (Mysecond(ptr) / 60)) / 60) * 2 * Pi);
}

double Myhour(struct tm *ptr)
{
	if (0 < ptr->tm_hour&&ptr->tm_hour < 12)
	{
		return((Pi / 2) - ((double)ptr->tm_hour + Mymin(ptr) / 60.0) / 12.0 * 2 * Pi);
	}
	else {
